            ...
            <q-menu>
              <q-list padding>
                ...
                <q-item class="text-black"
                  v-for="val in bookmarkStore.bookmarks.value.sort((a, b) => Number(a) - Number(b))" :key="val"
    ...
    <q-drawer v-model="rightDrawerOpen" side="right" overlay bordered :width="240" behavior="mobile">
      <q-scroll-area class="fit">
        <q-list>
          ...
          <q-item clickable class="text-black" v-for="(p, i) in pageData" :key="i" :to="getPageUrl(p.Index)"
            v-show="isFiltered(p.Title, i)">
            <q-item-section>{{ p.Index }}.{{ p.Title }}</q-item-section>
          </q-item>
        </q-list>
      </q-scroll-area>
      <!-- drawer content -->
