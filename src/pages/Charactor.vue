<template>
  <q-page padding>
    <q-card class="chara-card q-mx-auto">
      ...
      <q-card-section>
        <q-btn round fab-mini padding="xs" v-for="(c, i) in chara" :key="i" :to="'/charactor/' + c.name">
        ...
        <div v-for="(c, i) in chara" :key="i">
          <q-card flat v-if="name == c.name || name == 'all'">
            ...
            <q-card-section class="text-subtitle2" style="white-space:pre-wrap;">
              <span v-text="c.text"></span>
            </q-card-section>
            <q-card-section>
              <q-expansion-item class="text-center" label="関連作品" style="background-color:#f7f7f7;">
                <q-card class="text-left">
                  ...
                  <q-card-section>
                    ...
                    <div v-for="mainProd in getProductsBanner(c.cvArray, 'mainAct')" :key="mainProd">
  ...
  setup() {
    ...
    const getProductsBanner = computed(() => {
      return function (cvArray: Array<string>, prodType = 'mainAct') {
        const charaProdcts = db.products.filter(v => cvArray.some(cv => prodType == 'mainAct' ? v.mainAct.includes(cv) : v.subAct.includes(cv)));
