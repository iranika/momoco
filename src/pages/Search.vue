  ...
  <q-page padding>
    ...
    >
      <template v-slot:top-left>
        ...
        <div>
          <q-btn round push size="sm" v-for="c in chara" :key="c.name" @click="clickAvator(c.namae)">
...
import type { SearchDB } from 'src/stores/SearchStore';
...
export default defineComponent({
  ...
  setup() {
    ...
    function clickAvator(namae: string) {
      if (filter.value.includes(namae)) {
        filter.value = filter.value.replace(namae + ',', '');
        filter.value = filter.value.replace(namae, '');
      } else {
        filter.value += namae + ',';
      }
    ...
    const filterMethod = (
      ...
      const filterText = terms == null || terms === '' ? '' : String(terms);
      if (filterText === '') {
        return rows;
      }
    ...
    return {
      ...
      getSelectedString() {
        return selected.value.length === 0
          ? ''
          : `${selected.value.length} record${selected.value.length > 1 ? 's' : ''} selected of ${searchStore.products.value.length}`;
      },
    };
  },
});
