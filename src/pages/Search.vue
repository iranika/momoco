<template>
  <q-page padding>
    <q-table title="びゅあー検索㌼" :dense="$q.screen.lt.md" :grid="$q.screen.xs" :rows="searchStore.products.value"
      :columns="columns" row-key="No" :selected-rows-label="getSelectedString" selection="multiple"
      v-model:selected="selected" :pagination="pagenation" :filter-method="filterMethod" :filter="filter"
      style="max-width: 900px; margin: 0 auto;">
      <template v-slot:top-left>
        <div class="text-h5" style="margin-top:5px;">びゅあー検索くん㌼</div>
        <div>
          <q-btn round push size="sm" v-for="(c, i) in chara" :key="i" @click="clickAvator(i)">
...
const columns = [
  ...
  { name: 'Keyword', label: 'キーワード', align: 'left' as const, field: 'Keyword', format: (val: Array<string>) => val.join(',') },
  //{ name: 'Comment', label: 'コメント', align: 'left' as const, field: 'Comment' },
  { name: 'Link', label: 'リンク', align: 'left' as const, field: 'Links' }
]
