<template>
  <div class="viewer-content" v-scroll="scrolled">
    ...
    <div centered style="text-align:center; padding:10px">
      <q-btn rounded size="x-small" v-on:click="addContent()" color="white" class="text-black"
        v-if="pages.length - 1 > Math.max.apply(null, show)">▼続きを表示▼</q-btn>
    </div>
  </div>
</template>
  ...
  setup(props) {
    ...
    function initShow(page: string) {
      ...
      if (page === 'latest') {
        ...
        const match = pages.filter(v => String(v.Index) == page)[0];
        return match !== undefined ? [pages.indexOf(match)] : [0]
      } else if (pageInt <= 0) {
        return [0]
      } else {
